...
genotypes = pd.read_csv("final_genotypes_with_phenotypes.csv")
...
def get_participants_with_disease(keyword, icd10_codes, diagnosis_df, exclude_other_cancers=True):
    ...
    participants_with_disease.to_csv(f"{keyword}/{keyword}_cancer_diagnosis.csv", index=False)
...
def main(keyword, specific_drugs=True, exclude_other_cancers=True):
    ...
    prescription_survival_general_cancer_drugs.to_csv(f"{keyword}/{keyword}_general_cancer_drugs_survival{exclusion_suffix}.csv",
                                                      index=False)
    prescription_survival_keyword_specific_drugs.to_csv(
        f"{keyword}/{keyword}_specific_prescription_survival{exclusion_suffix}.csv", index=False)
    combined_df.to_csv(f'{keyword}/{keyword}_genotype_survival{exclusion_suffix}.csv', index=False)
...
if __name__ == "__main__":
    main(keyword="leukemia", specific_drugs=True, exclude_other_cancers=False)
